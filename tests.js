...
"Event 2, with comma","2025-07-29 11:00:00","2025-07-29 12:00:00","Description with ""quotes""","tentative","yes","yes"`;
    ...
    QUnit.test('formatDurationMinutes should format durations correctly', function (assert) {
        ...
        assert.equal(formatDurationMinutes(start, end30min), "30 min", "Correctly formats a 30-minute duration.");
        assert.equal(formatDurationMinutes(start, end90min), "1 hour 30 mins", "Correctly formats a 90-minute duration.");
        assert.equal(formatDurationMinutes(start, end2hours), "2 hours 0 min", "Correctly formats a 2-hour duration.");
    });
...
QUnit.test('Editing an overridden event twice should not create duplicates', function (assert) {
    const now = new Date('2025-07-28T08:00:00Z');
    fakeNow = now;
    ...
    customEvents = [firstEdit];
    renderEvents(now);

    let rendered = document.querySelectorAll('#events .event');
    assert.equal(rendered.length, 1, "After first edit, only 1 event is rendered.");
    assert.equal(rendered[0].querySelector('strong').textContent, 'UPDATED Team Meeting', "Summary is updated after first edit.");

    // 2. Second edit (simulating editing the 'UPDATED Team Meeting')
    const secondEdit = {
        ...firstEdit,
        summary: 'FINAL Team Meeting',
        bgColor: '#0000ff'
        // 'replacesId' is correctly carried over by the UI logic
    };
    // The UI logic would remove the firstEdit and add the secondEdit
    customEvents = [secondEdit];
    renderEvents(now);

    rendered = document.querySelectorAll('#events .event');
    assert.equal(rendered.length, 1, "After second edit, still only 1 event is rendered.");
    assert.equal(rendered[0].querySelector('strong').textContent, 'FINAL Team Meeting', "Summary is updated after second edit.");
    assert.equal(rendered[0].style.backgroundColor, 'rgb(0, 0, 255)', "Background color is updated after second edit.");
